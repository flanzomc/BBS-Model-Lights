...
public final class LightPanel extends UIFormPanel<Form> {
    ...
    @Override public void startEdit(Form form) {
        ...
        /* CML's bbs:light is deliberately tiny: just enabled + level. */
        if (form instanceof LightForm light) {
            UIToggle enabled = new UIToggle(IKey.constant("Enabled"), t -> light.enabled.set(t.getValue()));
            enabled.setValue(light.enabled.get());

            UITrackpad level = new UITrackpad(v -> light.level.set(v.intValue())).limit(0, 15).integer();
            level.setValue(light.level.get());

            options.add(enabled, UI.labelRow(IKey.constant("Light level"), level));
            options.resize();
            return;
        }
        ...
        if (form instanceof BlockForm) {
            ...
            options.add(enabled,
                UI.labelRow(IKey.constant("Light intensity"), level),
                UI.labelRow(IKey.constant("Breaking stage"), breaking));
        }
